...
- **ホットキーが他アプリにも届きます。** Windows 版は低レベルキーボードフックでキーを飲み込む（swallow する）ので、ホットキーを押しても前面アプリには何も送られません。X11 にはこれに相当する仕組みがなく、fire-speak が録音を開始したのと同時に、押したキーは前面のアプリにもそのまま渡ります。たとえば右 Alt を割り当てるとアプリ側のメニューが開いてしまうことがあります。**他アプリが使っていない組み合わせ**（例: `Ctrl+Alt+Space`）を「一般」タブで設定するのが実用的な回避策です。
...
- アプリ内のアップデート確認は「名前が `-setup.exe` で終わる**最初の**アセット」をダウンロード先にします。Windows arm64 のインストーラを `..._arm64_installer.exe` にリネームしているのはこのためです（x86_64 ユーザーに arm64 版を掴ませないため）
